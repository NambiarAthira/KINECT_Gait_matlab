%% A script that would save input (rgb and depth) as an AVI
% video with no compression. The output files are located in the local
% folder with the names:
% rgb.avi for the RGB image video. (RGB Colormap).
% depth.avi for the Depth image video (8-bit GrayScale).
%
close all;
clear all;
clc
...
cd(f_name);
% Name of the mat file, to store the details.
save(file, 'name_user', 'user_id'); % Storing the given details.
cd 'C:\kinect\knect_matlab';
...
cd(f_name);
copyfile('C:\kinect\knect_matlab\rgb.avi','rgb.avi')
copyfile('C:\kinect\knect_matlab\depth.avi','depth.avi')
